    ...
    $.fn.imageuploadify = function(opts) {
        ...
        this.each(function() {
            ...
            accept.forEach((item) => {
                ...
                acceptlist = acceptlist.concat(r);
            });
            ...
            const retrieveFiles = (files, input_name) => {
                ...
                for (let index = 0; index < files.length; ++index) {                    
                    if (!accept || compareMimeType(acceptlist, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])) {
            ...
            const readingFile = (id, file) => {
                ...
                if (file.type && file.type.search(/image/) != -1 && $(self).attr("filelist") === undefined) {
            ...
            const prepareRemoteFilesPreview = () => {
                ...
                if (remoteroot !== undefined) {
                    ...
                    for (var i = 0; i < remotefiles.length; i++) {
                        ...
                        let uri = remoteroot === '' ? remotefile : (remoteroot +  '/' + remotefile);
                        let content = remotefiletype !== 'file' ? uri : remotefile;
            ...
            const previewMediaLibraryImage = img => {
                if (!img) {
                    return;
                }
                ...
                var id = img.id;
                var url = img.thumb;
                var type = getMimeType(url);
                var input_name = $(self).attr('name') ;
                var content = url;
                var remotefile = extractFileName(url);

                $(self).attr('remotefile', `["${url}"]`);
                $(self).attr('remoteroot', '');
                $(self).attr('remotefiletype', 'gallery');
                ...
                $(self).closest("form").on("submit", function(event) {
                    ...
                    for (let input_name in totalFiles) {
                        ...
                        for (var i = 0; i < files.length; i++) {                         	               	
                            var file = files[i];
                            if (file.origin === 'remote') {
                                if (file.remotefiletype === 'gallery') {
                                    formData.append(`gallery_${input_name}_${i+1}`, file.file);
                                } else {
                                    break;
                                }
                            }
            ...
            self.previewOnlineImage = previewMediaLibraryImage;
        });
