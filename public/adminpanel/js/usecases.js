            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                      ...
                      return publish_icon + `<a href="/admin/usecases/edit/`+data+`" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Update">
  ...
  function(isConfirm) {
    if (isConfirm) {
      $.ajax({
        ...
        url: '/admin/usecases/delete/'+record_idx,
        method: 'post',
        success: function(res){
          if(res=="success"){
            window.location.href="/admin/usecases/"+communityIdx;
